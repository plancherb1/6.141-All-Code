...
img_ftr = cv2.imread(img1_path, cv2.IMREAD_GRAYSCALE)
img_scene = cv2.imread(img2_path, cv2.IMREAD_GRAYSCALE)

orb = cv2.ORB_create()
kp_scene = orb.detect(img_scene,None)
k_scene, d_scene = orb.compute(img_scene, kp_scene)
kp_ftr = orb.detect(img_ftr,None)
k_ftr, d_ftr = orb.compute(img_ftr, kp_ftr)

print '#keypoints in image1: %d, image2: %d' % (len(kp_ftr), len(kp_scene))
print '#keypoints in image1: %d, image2: %d' % (len(d_ftr), len(d_scene))

# match the keypoints
bf = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=True)
matches = bf.match(d_ftr, d_scene)
matches2 = bf.match(d_scene, d_ftr)
